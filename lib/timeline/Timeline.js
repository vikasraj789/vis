...
function Timeline (container, items, groups, options) {
  ...
  this.actualData = null;     // store copy of actual data Set
...
Timeline.prototype.searchGroupData = function (term, sortedOrder) {
  this.actualData = this.actualData ? this.actualData : this.groupsData.get();
  var filteredData = this.actualData;
  if (term.length) {
    var nestedGroups = {};
    filteredData = this.actualData.filter(function(data) {
      if((new RegExp("^" + term,"i").test(data.name)  || new RegExp(" "+term, "i").test(data.name)) && 
          data.hasOwnProperty('nestedInGroup')) {
        var groupId = data.nestedInGroup
        nestedGroups[groupId] = nestedGroups[groupId] ? nestedGroups[groupId] : [];
        nestedGroups[groupId].push(data.id);
        return data;
      }
      if(nestedGroups[data.id]) {
        data.nestedGroups = nestedGroups[data.id];
        return data;
      }
    });
  }
  this.setGroups(filteredData);
}
